...
# def main():
#     st.title("ISESA App")
#     option = st.selectbox("Select an option", ["Dropout", "Enrollment", "States with Facilities"])

#     if option == "Dropout":
#         show_dropout()
#     elif option == "Enrollment":
#         show_enrollment()
#     elif option == "States with Facilities":
#         show_states_with_facilities()

# def show_dropout():
#     st.write("This is the Dropout option")

# def show_enrollment():
#     st.write("This is the Enrollment option")

# def show_states_with_facilities():
#     facilities = st.multiselect("Select facilities", ["Water", "Electricity", "Computer", "Toilets"])
#     st.write("Selected facilities:", facilities)
...
def main():
    ...
    page_options = ["Index","Dropout Ratio", "Gross Enrollment Ratio", "States with Facilities"]
    selected_page = st.sidebar.selectbox("Select a page", page_options)
    
    if selected_page == "Index":
        index()
    elif selected_page == "Dropout Ratio":
...
def index():
    ...
    c2.image('https://mcmscache.epapr.in/post_images/website_350/post_29811255/full.jpg', use_column_width=True)
    st.markdown("""
    ...
    * **Data source:** [Indian School Education Statistics](https://data.gov.in/resources/stateut-wise-average-annual-drop-out-rate-2012-13-2014-15-ministry-human-resource
    """)
     ...
     # Add your code for displaying the index page here
...
def display_dropout_ratio():
    ...
    c1.dataframe(df_state[col], use_container_width=False, width=150)
